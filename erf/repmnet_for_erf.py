...
class repBlock(nn.Module):
    ...
    def switch_to_deploy(self):
        ...
        self.rep_conv = nn.Conv2d(in_channels=self.dw7x7[0].in_channels, out_channels=self.dw7x7[0].out_channels,
                                     ...
                                     padding=self.dw7x7[0].padding, dilation=self.dw7x7[0].dilation, groups=self.dw7x7[0].groups, bias=True)
...
class downsample(nn.Module):
    ...
    def switch_to_deploy(self):
        ...
        self.rep_conv = nn.Conv2d(in_channels=self.conv[0].in_channels, out_channels=self.conv[0].out_channels,
                                     ...
                                     padding=self.conv[0].padding, dilation=self.conv[0].dilation, groups=self.conv[0].groups, bias=True)
...
class RepMNet(nn.Module):
    def __init__(self, num_classes=102, drop_path_rate=0., dim=[96, 192, 384, 768], deeps=[3, 3, 9, 3],
        ...
        self.stage_4 = self._make_stage(dim[3], dim[3], deeps[3], dp_rates[15:18], layer_scale_init_value, is_downsample=False)
    ...
    def _make_stage(self, in_channels, out_channels, deep, dp_rates, layer_scale_init_value, is_downsample=True):
        ...
        for i in range(deep):
            blocks.append(repBlock(in_channels, dp_rates[i], layer_scale_init_value=layer_scale_init_value, deploy=self.deploy))
...
if __name__ == '__main__':
    ...
    net.eval()
    checkpoint = torch.load('../ft-3-76.3%/ckpt_epoch_286.pth', map_location='cpu')
    msg = net.load_state_dict(checkpoint['model'])
    print(msg)
    rep_net = model_convert(net, '../ft-3-76.3%/rep_best.pth')
    # net = RepMNet()
    # print(net)
